...
public class AccountServiceImpl implements AccountService {
    ...
    @Override
    public Account createAccount(AccountCreationDTO accountDto) throws SQLException, ResourceNotFoundException, IllegalArgumentException {
        // Validate user exists
        Optional<User> userOptional = userDao.getById(accountDto.getUserId());
        if (userOptional.isEmpty()) {
            throw new ResourceNotFoundException("User not found with ID: " + accountDto.getUserId() + ". Cannot create account.");
        }
        ...
        account.setAccountNumber(AccountGenerator.generateAccountNumber(accountDto.getUserId())); // Generate unique account number
        ...
        try {
            ...
            log.setDescription(String.format("New %s account created for user %s with initial balance %s. Account Number: %s",
                                account.getAccountType().name(), user.getUsername(), NumberUtils.formatCurrency(account.getBalance()), account.getAccountNumber()));
    ...
    public void closeAccount(int accountId) throws SQLException, ResourceNotFoundException {
        ...
        try {
            ...
            Account account = accountDao.getById(accountId).orElseThrow(
