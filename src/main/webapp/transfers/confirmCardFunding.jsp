...
<body>
    ...
    <div class="user-info">
        ...
        <main>
            <div class="transfer-confirm-container">
                <h2 class="confirm-title">Confirm Card Funding</h2>
                ...
                <div class="confirm-details">
                    <div class="account-row">
                        ...
                        <div>
                            <label>To Card</label><br>
                            <%= request.getParameter("toCard") != null ? request.getParameter("toCard") : "" %>
                        </div>
                    </div>
                    <div class="transfer-details">
                        <div><label>Transfer Details</label></div>
                        <div><label>Amount</label> <span><%= request.getParameter("fundingAmount") != null ? request.getParameter("fundingAmount") + " EUR" : "0.00 EUR" %></span></div>
                        <div><label>TBA Fee</label> <span>0.00 EUR</span></div>
                        <div><label>Description</label> <span><%= request.getParameter("fundingDescription") != null ? request.getParameter("fundingDescription") : "" %></span></div>
                        
                    </div>
                </div>
                
                <div class="button-group">
                    <button class="btn btn-back" onclick="window.location.href='cardFunding.jsp'">Back</button>
                    <button class="btn btn-confirm" onclick="confirmFunding()">Confirm</button>
                </div>
            </div>
        </main>
    </div>
    ...
    <script>
        function confirmFunding() {
            ...
            const toCard = "<%= request.getParameter("toCard") %>";
            const fundingAmount = "<%= request.getParameter("fundingAmount") %>";

            if (!fromAccount || fromAccount === "" || !toCard || toCard === "" || !fundingAmount || fundingAmount === "" || isNaN(fundingAmount) || fundingAmount <= 0) {
                alert("Transaction cannot be confirmed. Please ensure all required fields are filled with valid data.");
                return;
            }

            alert("Card funding done successfully!");
            ...
            setTimeout(() => {
                if (window.location.pathname.includes('confirmCardFunding.jsp')) {
