...
<body>
    ...
    <div class="user-info">
        ...
        <main>
            <div class="transfer-confirm-container">
                <h2 class="confirm-title">Transfer Between Accounts</h2>
                
                <div class="confirm-details">
                    <div class="account-row">
                        <div>
                            <label>Debit from</label><br>
                            <%= request.getParameter("fromAccount") != null ? request.getParameter("fromAccount") : "" %>
                        </div>
                        <div>
                            <label>Credit to</label><br>
                            <%= request.getParameter("toAccount") != null ? request.getParameter("toAccount") : "" %>
                        </div>
                    </div>
                    <div class="transfer-details">
                        <div><label>Transfer Details</label></div>
                        <div><label>Amount to transfer</label> <span><%= request.getParameter("transferAmount") != null ? request.getParameter("transferAmount") + " EUR" : "0.00 EUR" %></span></div>
                ...
                <div class="button-group">
                    <button class="btn btn-back" onclick="window.location.href='betweenAccounts.jsp'">Back</button>
    ...
    <script>
        function confirmTransfer() {
            ...
            const toAccount = "<%= request.getParameter("toAccount") %>";
            const transferAmount = "<%= request.getParameter("transferAmount") %>";

            if (!fromAccount || fromAccount === "" || !toAccount || toAccount === "" || !transferAmount || transferAmount === "" || isNaN(transferAmount) || transferAmount <= 0) {
            ...
            setTimeout(() => {
                if (window.location.pathname.includes('confirmTransfer.jsp')) {
